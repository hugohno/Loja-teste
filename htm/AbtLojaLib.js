...
var sF$=(function(){
  ...
  function fnShowEconomy(ProdPrice,ProdPriceOri){
    if(ProdPrice!=ProdPriceOri)document.write("<font style='font-size:16px;display:block;margin:10px 0;' color=#6f9e45>Economize <b>"+FormatPrice(ProdPriceOri-ProdPrice,'R$')+"</b> ("+fnFormatNumber(((ProdPriceOri-ProdPrice)/ProdPriceOri)*100)+"%)</font>");
      ...
      //Função para mostrar valor economizado em produtos em promoção
function fnShowEconomyHome(ProdPrice, ProdPriceOri, vestoque, idProdHome){
  oElem = document.getElementById('idElEstoqueProm'+idProdHome);

   if (ProdPrice != ProdPriceOri) {
//    oElem.innerHTML = '<div class=DivHomeEconomia>Economize <b>'+FormatPrice(ProdPriceOri-ProdPrice,'R$')+'</b> ('+fnFormatNumber(((ProdPriceOri-ProdPrice)/ProdPriceOri)*100)+'% OFF) </div>';
    oElem.innerHTML = '<div class=DivHomeEconomia> <b>'+fnFormatNumber(((ProdPriceOri-ProdPrice)/ProdPriceOri)*100)+'% OFF </b> </div>';
  }
}
...
function maskdata(){
  ...
  document.getElementById('P2DataNasc').onkeypress = function(){
      var P2DataNasc = document.getElementById('P2DataNasc').value;
                 if (P2DataNasc.length == 2){
                  P2DataNasc = P2DataNasc + '/';
                  document.Form1.P2DataNasc.value = P2DataNasc;
      return true;              
              }
              if (P2DataNasc.length == 5){
                  P2DataNasc = P2DataNasc + '/';
                  document.Form1.P2DataNasc.value = P2DataNasc;
                  return true;
              }
 ...
 function masktel(){
  ...
  document.getElementById('P2Telefone').onkeypress = function(){
      var P2Telefone = document.getElementById('P2Telefone').value;
                 if (P2Telefone.length == 0){
                  P2Telefone = P2Telefone + '(';
                  document.Form1.P2Telefone.value = P2Telefone;
      return true;              
              }
                 if (P2Telefone.length == 3){
                  P2Telefone = P2Telefone + ')';
                  document.Form1.P2Telefone.value = P2Telefone;
      return true;              
              }
              if (P2Telefone.length == 8){
                  P2Telefone = P2Telefone + '-';
                  document.Form1.P2Telefone.value = P2Telefone;
                  return true;
              }
