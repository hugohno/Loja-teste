...
function AlteraTextoSitPedido(){
      ...
      if(sitAprovado)sitAprovado.innerHTML="Status pedido Aprovado";
      else{
...
if(FC$.Page=="Products"||FC$.Page=="Cart")FCLib$.onReady(FCLib$.enlargeQtyInput);
...
function fnExibeHistoricoNaveg(oDocXML){
  ...
  if(oPageHistory){
    ...
    if(sPageHistory!=""){
      sPageHistory="<div class='histBorder'><div class='pageHist'>Histórico de navegação</div><div class='pageHistMain'>"+sPageHistory+"</div></div>";
...
function fnLoginShowUserName(user){
  fnLoginUserName(user.firstName,user.pictureURL);
}
...
/*Strike para prmoção na home  */
function Strike(){

var oldprice = document.getElementsByClassName('old-price')[0];
console.log(oldprice.replace(/[^\d]+/g,''));
oldprice.innerHTML = "teste"
}

/*Função para mensagem de frete grátis do carrinho*/
function Frete_gratis(){
 var cesta = parseFloat(document.getElementsByClassName('FCPriceValue')[1].innerText);
 var alvo = 150;
 var falta = alvo - cesta;
 if (cesta > alvo) {
  document.getElementById('FCCartRightCupomBut').innerHTML +="<div  style='font-size: 18px;margin-top: 10px;border: 4px dashed #696969;padding: 5px;background-color: #FF0000;color: #fff;'><b>Frete Gratuito!</b></div>"
 }
 else{document.getElementById('FCCartRightCupomBut').innerHTML +="<div  style='font-size: 18px;margin-top: 10px;border: 4px dashed #696969;padding: 5px;background-color: #FF0000;color: #fff;'>Faltam <b>" + falta + "</b> reais para o frete ser gratuito </div>"}
}

//Esconder boleto para clientes PJ
function CliPJ(){
 if (document.getElementById('P2CPF').length > 11) {

    document.getElementById('idPagto1FC').style.display = "none";
 }
} 