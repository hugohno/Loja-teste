...
function FC_fRedefineOpcoes(IDProduto,aDados,aDistintos,iCampo,ValorCampo){
  ...
  for(var j=0;j<aDistintos[iCampo].length;j++){
    ...
    if(aDistintos[iCampo][j]!=ValorCampo){
      ...
      FC_Option.style.backgroundImage="url('"+FC$.PathImg+"ICCheckGRIDOff.svg')";
      FC_Option.style.backgroundRepeat="no-repeat";
      FC_Option.style.paddingTop = "15px";
      FC_Option.style.paddingBottom = "5px";
    }
    else{
      FC_Option.style.borderWidth='2px';
      FC_Option.style.borderStyle='solid';
      FC_Option.style.borderColor='#888888';
      FC_Option.style.backgroundImage="url('"+FC$.PathImg+"ICCheckGRIDOn.svg')";
      FC_Option.style.backgroundRepeat="no-repeat";
      FC_Option.style.paddingTop = "15px";
      FC_Option.style.paddingBottom = "5px";
    }
  ...
  if(iCampo==0 && aDados.length>1){
    ...
    MagicZoomPlus.refresh();   //atualizar img zoom 
  }
...
//    document.getElementById('RGBCorProduto').innerHTML="<span style='width:15px;background-color:#"+sRGBCor+"'>&nbsp;&nbsp;</span>";
  ...
  if(NivelSeguinte!=-1){  //habilita um nível abaixo
    document.getElementById("FC_idNivel_"+ NivelSeguinte +"_"+ IDProduto).className="NivelOn gridLevel";
    // document.getElementById("FC_idNivel_"+ NivelSeguinte +"_"+ IDProduto).className="gridLevel";
    for(var j=0;j<aDistintos[NivelSeguinte].length;j++){  //habilita opções do nível abaixo
      if(aDistintos[NivelSeguinte][j]!=''){
        ...
        if(bExisteOpcao){FC_Option.style.backgroundColor='#CCCCCC';FC_Option.style.backgroundImage="url('"+FC$.PathImg+"ICCheckGRIDOff.svg')";}
  ...
  if(NivelSeguinte==-1){  //verifica o ID e o estoque do produto
    ...
    if(oSubProd.Existe){
      ...
      if(oSubProd.PrecoNum==0 && oSubProd.PrecoOri==0){
        ...
        oBotao.style.backgroundImage="url('"+FC$.PathImg+"BotGridConsultenos.svg')";
      ...
      var ParcelamentoJS=MostraMaxParcelaJS(oSubProd.PrecoNum,0);
        ...
        if(oSubProd.iEstoque>0){
          ...
          oBotao.style.backgroundImage="url('"+FC$.PathImg+"BotGridComprar.svg')";
          ...
          oBotao.style.backgroundImage="url('"+FC$.PathImg+"BotGridEsgotado.svg')";
          oBotao.onclick=new Function('MostraDispCaptcha('+FC$.IDLoja+',"'+oSubProd.IDSubProduto+'");');	//Função para aviso de disponibilidade
        }
    ...
    oBotao.style.backgroundImage="url('"+FC$.PathImg+"BotGridSelecione.svg')";
    oBotao.disabled=true;
    oBotao.onclick=null;
  }
}
...
function FC_CompraSubProd(IDSubProd,Cor,Adicional1,Adicional2,Adicional3,AdicionalD1,AdicionalD2,AdicionalD3){
  ...
  var sNomeCor=aNomeRGB[0];
  var sURLCompra='AddProduto.asp?IDLoja='+FC$.IDLoja+'&IDProduto='+ IDSubProd;
...
function FC_ConsultenosSubProd(IDSubProd,Cor,Adicional1,Adicional2,Adicional3,AdicionalD1,AdicionalD2,AdicionalD3){
  ...
  var aNomeRGB=Cor.split(SeparadorRGBCor);
  var sNomeCor=aNomeRGB[0];
  var sURLConsultenos='FaleConosco.asp?IDLoja='+FC$.IDLoja+'&Assunto=Consulta%20sobre%20produto ('+ IDSubProd +') ';
...
//Inicializa arrays e exibe opções de IDProduto
  ...
  for(var i=0;i<FC_QtdDescritores;i++){
    if(i==NivelSeguinte){FC_sClass="NivelOn"; FCLevel="gridLevel"; var sDisabled="";}
    else{FC_sClass="NivelOff";var sDisabled=" disabled"; FCLevel="gridLevel";}
    document.write("<div id=FC_idNivel_"+ i +"_"+ IDProduto +" class='"+ FC_sClass+' '+FCLevel+i+" col-small-12 col-medium-12 col-large-12 col-xlarge-12'><span class='descriChoice_"+i+"'>"+(i+1)+"° Passo</span><ul class=lstColor>");
    // document.write("<table><tr><td><div id=FC_idNivel_"+ i +"_"+ IDProduto +" class='"+ FC_sClass +' '+ FCLevel +"'><ul class=lstColor>");
    ...
    if(aDistintos[i].length>0){
      for(var j=0;j<aDistintos[i].length;j++){
        ...
        if(i==0){  //separa nome e RGB da cor
          ...
          //sValor="<span style='width:27px;background:url("+FC$.PathPrd+"cores/"+aNomeRGB[0]+".gif) no-repeat;'>&nbsp;&nbsp;&nbsp;&nbsp;</span>"; //EXEMPLO DE IMAGEM PARA CADA COR
  ...
  document.write("<div class='gridLevel col-small-12 col-medium-12 col-large-12 col-xlarge-12'><span class='descriChoice_"+i+"'>"+(i+1)+"° Passo</span><div id=RGBCorProduto></div><div id=CorProduto></div><div id=TamanhoProduto></div><input type=button class=BotComprarGrid name=FC_oBotao_"+ IDProduto +" id=FC_oBotao_"+ IDProduto +" onClick='' value='' disabled></div>");
